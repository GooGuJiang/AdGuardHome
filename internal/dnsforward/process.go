...
func (s *Server) processUpstream(dctx *dnsContext) (rc resultCode) {
	...
	if pctx.Res != nil {
		...
		log.Debug("dnsforward: dhcp client hostname %q was not filtered", q.Name[:len(q.Name)-1])
		pctx.Res = s.genNXDomain(req)
...
// isDHCPClientHost returns a hostname from question, if the request is for a
// DHCP client's hostname when DHCP is enabled, and otherwise an empty string.
func (s *Server) isDHCPClientHost(q *dns.Question) (reqHost string) {
